<template>
  <div class="d-flex">
    <div class="d-flex double-select">
      <div class="select-container select-container-left border">
        <input-select-single
          selectBtnClassNames="border-0"
          :resetBtn="false"
          :list="periodList"
          :disabled="disabled"
      ...
      <date-pick
        ...
        <template #action-row="{ internalModelValue, selectDate, closePicker }">
          <div class="w-100 mt-4">
            <div
              ...
              <input
                class="form-control"
                :class="{ 'border-danger': !inputDates.start.isValid }"
                style="width:138px"
                type="text"
                :value="isInputActive ? inputDates.start.value : syncDate(internalModelValue[0], 'start')"
                @input="inputDate(internalModelValue, 'start', $event)"
                @keydown.stop />
              <span class="mx-2">—</span>
              <input
                class="form-control"
                :class="{ 'border-danger': !inputDates.end.isValid }"
                style="width:138px"
                type="text"
                :value="isInputActive ? inputDates.end.value : syncDate(internalModelValue[1], 'end')"
                @input="inputDate(internalModelValue, 'end', $event)"
                @keydown.stop />
            </div>
            <div class="d-flex align-items-center justify-content-between w-100 pt-4 mt-4 border-top">
              <button
                class="btn border"
                @click="closePicker">
                Сбросить
              ...
              <button
                ...
                @click="clickSelectDate(selectDate)">
                Готово
              </button>
            </div>
          </div>
        ...
        <template #trigger>
          <input
            ...
            type="text"
            :value="displayDate"
            class="form-control select-container select-container-right text-cut"
...
<script>
  ...
  export default {
    ...
    setup() {
      ...
      const period = ref(1);
      const periodList = [
        {id: 1, name: 'Последняя неделя'},
        {id: 2, name: 'Последний месяц'},
        {id: 3, name: 'Текущий год'},
        {id: 4, name: 'Выбрать период'}
      ];
      const rows = [
        {id: 'day', name: 'По дням'},
      ...
      const getDate = (num) =>
        num
          ? `${num.getDate()} ${monthsNames[num.getMonth()]}`
          : '...';
      ...
      const clickSelectDate = (selectDate) => {
        const d = JSON.stringify(date.value);
        selectDate();
        if (JSON.stringify(date.value) !== d) {
          isShow.value = true;
          period.value = 4;
      ...
      const inputDate = (internalModelValue, str, e) => {
        ...
        const d = [
          +day,
          monthsNames.includes(month?.toLowerCase()) ? monthsNames.indexOf(month.toLowerCase()) + 1 : +month,
      ...
      const inputDates = ref({
        start: { isValid: true, value: getDate(store.calendar.date[0]), isActive: false },
        end: { isValid: true, value: getDate(store.calendar.date[1]), isActive: false }
      });
