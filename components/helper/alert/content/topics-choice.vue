  ...
  <div class="ac-topic-choice">
    ...
    <div
      ...
      <div class="ac-topic-choice-info-content">
        ...
        <div
          v-for="(subject, index) in selectedClasses"
          :key="index"
          :title="subject + ' класс'"
          class="ac-topic-choice-info-tag">
      ...
      <div
        ...
        <div
          ...
          <input-checkbox
            :value="item"
            :label="item.name"
            :id="'topic-' + generateString()"
            :checked="topicChoiceChecked(item)"
            @change="topicChoiceChange">
            <template
              v-if="item.items?.length"
              #labelAfter>
              <div class="ac-topic-choice-content-items-count">
                {{ item.items.length }}
              </div>
            </template>
          </input-checkbox>
    ...
    <div class="ac-topic-choice-border mt-4 mb-3"/>
    <div class="ac-topic-choice-footer">
      <div
        @click="resetSelectedTopics"
        class="ac-topic-choice-footer-reset">
        Отменить
      </div>
      <div
        :class="[{'hidden': !selectedTopics.length}]"
        class="ac-topic-choice-footer-counter">
        Выбрано
        <div class="ac-topic-choice-footer-counter-count">
...
<script setup>
  ...
  const selectedSubjects = ref(props.data.filters?.subjects?.value || []);
  const selectedClasses = ref(props.data.filters?.classes?.value || []);
  ...
  const subjectsList = computed(() => {
    if (props.data.filters?.subjects?.data?.length) {
      return props.data.filters.subjects.data;
    }

    return useCommonStore().filtersData.data.subjects;
  });
  const classesList = computed(() => {
    if (props.data.filters?.classes?.data?.length) {
      return props.data.filters.classes.data;
    }
  ...
  onBeforeMount(() => {
    selectedTopics.value = props.data.topics || [];
  });
  onMounted(() => {
  ...
  const topicChoiceChecked = (topic) => {
    return selectedTopics.value.find((_topic) => _topic === topic.id) !== undefined;
  };
  const topicChoiceChange = (value, $event) => {
    $event.target.checked
      ? selectedTopics.value.push(value.id)
      : selectedTopics.value.splice(
        selectedTopics.value.indexOf(value.id),
        1
      );
  };
