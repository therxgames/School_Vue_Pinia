<template>
  <div class="task-versionsDifference">
    <loader
      v-if="!isInitialized"
      class="task-versionsDifference-loader"
      :listener="!isInitialized"/>
    <div
      v-if="isInitialized"
      class="task-versionsDifference-content">
      <div v-if="differenceCount">
        <b class="font-weight-500">Изменения относительно {{ compareWithNumber }} версии:</b>
        <div v-if="difference.task_name">
          Изменено наименование задания
        </div>
        <div v-if="difference.task_text">
          Измененен текст задания
        </div>
        <div v-if="difference.answer_variants">
          Изменено количество вариантов ответа
        </div>
        <div v-if="difference.answer_right">
          Измененен верный ответ
        </div>
      </div>
...
<script setup>
  const props = defineProps({
    taskParentId: {type: Number, required: true},
    versionOne: {type: Number, required: true}
  });
  ...
  const compareWithId = computed(() => {
    ...
    if (startVersion && startVersion.version_number !== 1) {
      return versionByVersionNumber.value[startVersion.version_number - 1].id;
    }

    return 0;
  });
  const compareWithNumber = computed(() => {
    return versionById.value[compareWithId.value]?.version_number;
  });
  ...
  const fetchDifference = async () => {
    ...
    try {
      const response = await useApi(`/api/task/version_differences`, {
        ...methodGet,
        query: {
          task_id_cur: props.versionOne,
          task_id_prev: compareWithId.value
        }
      });
      if (response.data.done) {
...
<style lang="scss">
  ...
  .task-versionsDifference {
    display: inline-flex;
  }

  .task-versionsDifference-content {
  ...
  .task-versionsDifference-loader {
